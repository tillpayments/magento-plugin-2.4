...
namespace TillPayments\TillPaymentsPlugin\Helper;
...
class Data extends AbstractHelper
{
    ...
    const PAYMENT_REDIRECT_CANCEL = 'tillpayments/payment/redirect?status=cancel';
    const PAYMENT_REDIRECT_ERROR = 'tillpayments/payment/redirect?status=error';
    const PAYMENT_CALLBACK = 'tillpayments/payment/callback';
    ...
    const PROD_URL = 'https://gateway.tillpayments.com/api/v3/transaction/';
    const SANDBOX_HOST_URL = 'https://test-gateway.tillpayments.com/';
    const PROD_HOST_URL = 'https://gateway.tillpayments.com/';
    ...
    {
        return $this->getConfigData($field, 'tillpayments/general', $storeId);
        /* if (in_array($field, self::ENCRYPTED_CONFIG_FIELDS)) {
            return $this->_encryptor->decrypt($this->getConfigData($field, 'tillpayments/general', $storeId));
        } else {
            return $this->getConfigData($field, 'tillpayments/general', $storeId);
        } */
    }
    ...
    {
        return '/api/v3/transaction/' . $this->getTillPaymentsConfigValue(self::MERCHANT_API_KEY) . $transactionType;
    }
     ...
     * @return string
     */
    public function getTillPaymentsConfigValue($configField): string
    {
        $scope = ScopeInterface::SCOPE_STORE;
    ...
    {
        $signatureKey = $this->getTillPaymentsConfigValue(self::MERCHANT_SECRET_KEY);
    ...
    {
        return $this->scopeConfig->getValue('tillpayments/general/sandbox', ScopeInterface::SCOPE_STORE);
    }
