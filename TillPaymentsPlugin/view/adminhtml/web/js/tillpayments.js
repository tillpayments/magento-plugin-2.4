...
define([
  ...
  'mage/translate',
  'TillPayments_TillPaymentsPlugin/js/validator'
  ...
  return Class.extend({
    ...
    defaults: {
      $selector: null,
      selector: 'edit_form',
      container: 'payment_form_tillpayments_creditcard',
      active: false,
      scriptLoaded: false,
      tillpayments: null,
      token: null,
    ...
    onActiveChange:function (isActive) {
      if (!isActive) {
        this.$selector.off('submitOrder.tillpayments_creditcard');
        return;
      }
     ...
     */
    initTillPayments: function () {
      $(".tillpayments-error").hide();   
    ...
    enableEventListeners: function () {
      this.$selector.on('submitOrder.tillpayments_creditcard', this.submitOrder.bind(this));
    },
    ...
    /**
     * Convert card information to tillpayments token
     */
    ...
    this.paymentJs.tokenize(
        ...
        function(token, cardData) {
            $('#tillpayments_creditcard_transactionToken').val(token);
            defer.resolve();
        }, 
